...
def f(arg_for_seed, nb_of_elements, max_element):
    ...
    f_list = []
    for value in L:
        str_l.append(str(value))
    for value in str_l:
        first.append(value[0])
    first = sorted(first)
    for value in first:
        q = first.count(value)
        if [q, value] not in f_list:
            f_list.append([q, value])
    for value in f_list:
        if value[0] == 1:
            print(f"{value[0]} element starts with {value[1]}")
        else:
            print(f"{value[0]} elements start with {value[1]}")
